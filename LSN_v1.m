...
W         =64;%CANTIDAD MAXIMA DE RANURAS
...
for cic1=1:Ciclos   %%CORRE TANTOS CICLOS COMO DESEEMOS
    ...
    if(length(nod)==1)%%COMPROBAMOS QUE SOLO HAYA UN NODO GANADOR PARA LA RANURA ELEGIDA
        ...
        if(i>1)%%VERIFICAMOS QUE NO NOS ENCONTRAMOS EN EL NODO SINK
            if(rx_flag(nod)==false)%%SI EL BUFFER ESTA LLENO EL NODO RECEPTOR NO DESPIERTA
            ...
            retardo_pkt= T_sim+DIFS+Ro*ranura(nod)+RTS+CTS+DATA+ACK+3*SIFS-pkt.T_arribo;
    ...
    while(any(T_arribos(:)<T_sim))
        [T_arribos, nodos,Pkt_stats] = Actualiza_nodo(T_arribos,nodos,pkt_status,T_sim,Lambda,K);
    end
end
...
grid on; hold on;
plot(retardo_promedio,'linewidth',2,'color','r');
T_arribos
retardo_promedio
